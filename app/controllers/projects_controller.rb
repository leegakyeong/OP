class ProjectsController < ApplicationController
    ...
    def search
        # 프로젝트 이름으로 검색
        title = params[:title]
        # @result = Project.find_by_title(title) 이렇게 하면 조건을 만족하는 첫 번째 것 하나밖에 가져올 수 없음
        # @results = Project.where(title:title)
        @results = []

        Project.all.each do |p|
            if p.title.include? title # 레일즈 3.1.0부터 include? 대신 in? 써도 된다더니 안 됨!!
                @results.push(p)
            end
        end
